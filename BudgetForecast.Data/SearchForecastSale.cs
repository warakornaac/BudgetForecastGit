...
namespace BudgetForecast.Data
{
    public class SearchForecastSale : MsSQL
    {
        ...
        public List<StoreSearchForecastSaleModel> GetStoreSearchForecastSale(string slmCode, string[] cusCode, string[] stkSec, string year, int flg, int month = 0, int flgBudget = 1)
        {
            ...
            var table = GetData(CmdStore("P_Search_Forecast_Sale_Dev_1", p));
            return ConvertExtension.ConvertDataTable<StoreSearchForecastSaleModel>(GetData(CmdStore("P_Search_Forecast_Sale_Dev_1", p)));
        }
    }
}
